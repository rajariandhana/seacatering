...
const PlanRadio=({ plan }: { plan: Plan })=>{
  return (
    <Radio 
      ...
      value={plan.key}
    >
      <div className="flex flex-wrap items-center justify-between w-[280px] md:w-[400px]">
        <h3 className={`${domine.className} text-md md:text-xl font-semibold mb-0 text-primary ${plan.key==='vegetarian'?'text-green-600':''}`}>{plan.name}</h3>
